    ...
    axios
      ...
      .then(() => {
        setHabits((prevHabits) =>
          prevHabits.map((h) => (h.id === id ? { ...h, done: !done } : h))
        );
      })
      .catch((err) => {
        console.log(err);
  ...
  return (
    <Box>
      <Registry>
        <h1>{title}</h1>
        <Sequence >
          Sequência atual: <strong>{currentSequence}</strong> dias
        </Sequence>
        <Record >
          Seu recorde: <strong>{recordSequence}</strong> dias
        </Record>
      </Registry>
      <Button $done={done} onClick={toggleCheckHabit}>
        <CustomCheckIcon sx={{ fontSize: 50 }} />
      </Button>
    </Box>
  );
}
